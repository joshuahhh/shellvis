...
const registry = new vsctm.Registry({
  onigLib: (async () => {
    const wasmbin = await fsP.readFile('./node_modules/vscode-oniguruma/release/onig.wasm');
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const token of tokens) {
      if (token.settings.foreground) {
        lineColored += withColor(line.substring(token.startIndex, token.endIndex), token.settings.foreground);
      } else {
