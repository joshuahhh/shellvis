...
export async function afterRun(sandbox: Sandbox): Promise<DeltaLogEntry[]> {
  ...
  async function walkCreationsAndModifications(pathInUpper: string) {
    ...
    for await (const dirent of await fsP.opendir(path.join(upperDir, pathInUpper))) {
      if (dirent.isDirectory()) {
        await walkCreationsAndModifications(path.join(pathInUpper, dirent.name));
...
async function applyDeltaLogEntry(entry: DeltaLogEntry, upperDir: string, sandboxUnionDir: string) {
  switch (entry.event) {
    case 'deletedDir':
      await fsP.rm(path.join(sandboxUnionDir, entry.path), { recursive: true, force: true });
      break;
    case 'deletedFile':
      await fsP.rm(path.join(sandboxUnionDir, entry.path));
      break;
    case 'newDir':
      await fsP.mkdir(path.join(sandboxUnionDir, entry.path), { recursive: true });
      break;
    case 'modifiedFile':
      await fsP.copyFile(path.join(upperDir, entry.path), path.join(sandboxUnionDir, entry.path));
      break;
...
export function makeDeltaLogEntryAbsolute(entry: DeltaLogEntry, rootDir: string): DeltaLogEntry {
  return {
    ...entry,
