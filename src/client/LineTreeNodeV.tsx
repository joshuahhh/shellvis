...
import { ForIteration, mkExecId, Trace } from "../shared/execution.js";
import {
  getNodeId,
  LineTreeNode,
  nodePosInfo,
...
import {
  addDecorationsToLineHelper,
  addDecorationsToLineStarter,
  Decoration,
} from "./decorations.js";
...
export const LineTreeNodeV = memo(function LineTreeNodeV(
  props: LineTreeNodeVProps,
) {
  const {
    script,
    trace,
  ...
  if (doThisOneThingForMeAndStickThisIntoTheFirstRHS && node.type !== "line") {
    console.warn(
      "doThisOneThingForMeAndStickThisIntoTheFirstRHS is getting lost",
    );
  }
...
const LineNum = memo(function LineNum(props: { children?: number }) {
  ...
  const onClick = useCallback(() => {
    if (!hvContextUP || !children) {
      return;
    }
    if (
      selections.some(
        (selection) =>
          selection.start.line <= children - 1 &&
          children - 1 <= selection.end.line,
      )
    ) {
...
const DeadLineTreeNodeV = memo(function DeadLineTreeNodeV(
  props: DeadLineTreeNodeVProps,
) {
  ...
  if (node.type === "line") {
    ...
    return (
      <Line $inPlace={detailsMode === "in-place"} className="text-gray-400">
        <LineNum>{node.lineNumStart}</LineNum>
        <LineContents data-line={node.lineNumStart - 1}>
          {node.line}
        </LineContents>
      </Line>
    );
...
const LineV = memo(function LineV(props: LineVProps) {
  ...
  const abbreviate =
    abbreviateInfo === "always"
      ...
      : abbreviateInfo === "never"
        ...
        : !isHovered &&
          !selections.some(
            (selection) => selection.start.line <= i && i <= selection.end.line,
          );

  let [nodes, starts, ends] = addDecorationsToLineStarter(line);
  const colorDecorations: Decoration[] = script.tokensByLine![i].flatMap(
    (token) => {
  ...
  return (
    <Line $inPlace={detailsMode === "in-place"}>
      <LineNum>{i + 1}</LineNum>
      <LineContents data-line={i} className={lineContentsClassName}>
        {nodes}
      </LineContents>
      {doThisOneThingForMeAndStickThisIntoTheFirstRHS && (
      ...
      {detailsMode === "grid" && callExprsOnLine.length > 0 && (
        ...
        <div
          className="grid grid-cols-subgrid col-span-2 col-start-3"
          ref={setElem}
        >
          <LineCalls
            style={{ paddingLeft: depth * 20 }}
            className="row-start-1 col-start-1 my-1 mx-1"
          >
            <div
              className={clsy(
                "flex flex-wrap gap-1",
                !abbreviate && "invisible",
              )}
            >
...
const LoopHeader = memo(function LoopHeade(props: {
  ...
  const { iteration, iterationIdx, setIterationIdx, varName, numIterations } =
    props;
  ...
  return (
    <ForLoopIterationHeader>
      <Slider
        ...
        step={1}
        value={iterationIdx}
        onChange={(_, newValue) => setIterationIdx(newValue as number)}
...
const LockSize = memo(function LockSize(props: {
  ...
  return (
    <div style={{ ...wrapperStyle }} ref={setWrapper}>
      {children}
    </div>
  );
});
...
const WhileLoopCondAndBodyV = memo(function WhileLoopCondAndBodyV(props: {
  ...
  const whileInfo =
    trace.whileInfos[mkExecId({ context, nodeId: whileNodeId })];
  ...
  let header = !whileInfo ? null : numIterations === 0 ? (
    <LineCalls style={{ paddingLeft: depth * 20 }} className="col-start-3">
      <ForLoopIterationHeader>
        <div>{forIndent}</div>
        <ForLoopIterationHeaderLabel className="border border-blue-500 text-blue-500">
