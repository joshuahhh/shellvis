...
import {
  ...
  ShellVarChange,
  diffShellVars,
  shellVarChangeVarName,
} from "../shared/typeset.js";
...
const ReactDiffViewer =
  diffViewerModule as any as typeof diffViewerModule.default;
...
export const CallOnGridV = memo((props: CallOnGridVProps) => {
  ...
  return (
    <div
      data-dbg="CallOnGridV"
      className="inline-flex flex-col items-start min-w-0"
    >
      {showCodeLabel && (
        <div
          data-dg-name="CallOnGridV code label"
          className="text-gray-500 text-xs w-0 min-w-full whitespace-nowrap overflow-hidden text-ellipsis font-mono"
        >
  ...
  return (
    execInfo && (
      ...
      >
        ...
        <div className="p-1">{providerOutputs}</div>
      </div>
    )
  );
});
...
function getInfoProviderOutputs(
  ...
  return infoProviders
    ...
    .map(([providerOutput, i]) => (
      <Fragment key={i}>{providerOutput}</Fragment>
    ));
}
            ...
            if (count > 0) {
              return (
                <Fragment key={event}>
                  ...
                  <InfoEntryContents>{count}</InfoEntryContents>
                </Fragment>
              );
            } else {
              return null;
...
infoProviders.push(({ abbreviate, execInfo }) => {
  if (execInfo && execInfo.varsEnterStr && execInfo.varsExitStr) {
    const varsDiff = diffShellVarsFromStr(
      execInfo.varsEnterStr,
      execInfo.varsExitStr,
    );
            ...
            if (count > 0) {
              return (
                <Fragment key={type}>
                  ...
                  <InfoEntryDetails>{count}</InfoEntryDetails>
                </Fragment>
              );
            } else {
              return null;
    ...
    contents = (
      <>
        <Tooltip
          title={
            <H>
              was <C>{change.oldVar.attributes}</C>
            </H>
          }
          placement="top"
          arrow
        >
          <div>
            <C>{change.newVar.name}</C> attributes:{" "}
            <C>{change.newVar.attributes}</C>
          </div>
        </Tooltip>
      </>
    );
...
const diffShellVarsFromStr = weakMapCache2(
  (varsEnterStr: RawString, varsExitStr: RawString) => {
