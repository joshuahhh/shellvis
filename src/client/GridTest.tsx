...
export const GridTest = memo(() => {
  ...
  const [ gTC, setGTC ] = useState('fit-content(0%)_minmax(0,max-content)_minmax(min-content,max-content)_1fr');
   ...
   {widths.map((width, i) =>
      <div key={i} style={{width, maxWidth: width}} className='bg-gray-600'>
        <div className='inline-grid grid-cols-[fit-content(0%)_minmax(0,max-content)_minmax(min-content,max-content)_1fr]'>
          ...
          <TestBlock className='mt-6 bg-orange-500'>helloooooooooooo there</TestBlock>
...
const TrackLabels = memo((props: {
  ...
  return Array.from({ length: numTracks }).map((_, i) => {
    return <div key={i} className='text-gray-300 text-sm text-center relative h-6'>
...
const TestBlock = memo((props: {
  ...
  const [ widthStats, setWidthStats ] = useState<ReturnType<typeof getWidthStats> | null>(null);
  const setDiv = useCallback((elem: HTMLDivElement | null) => {
    if (!elem) { return; }
    setWidthStats(getWidthStats(elem));
  }, []);
  return <div ref={setDiv} className={twMerge('h-6 rounded flex bg-slate-500 relative overflow-y-clip', className)}>
    {props.children}
    {widthStats && <div className='absolute w-px h-full bg-gray-300' style={{left: widthStats.minContent - 1}}/>}
    {widthStats && <div className='absolute w-px h-full bg-gray-300' style={{left: widthStats.maxContent - 1}}/>}
  </div>;
});

export function getWidthStats(elem: HTMLElement): { minContent: number, maxContent: number } {
