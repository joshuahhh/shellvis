...
export const TraceV = memo((props: TraceVProps) => {
  ...
  const [backgroundElem, setBackgroundElem] = useState<HTMLDivElement | null>(
    null,
  );

  const [backgroundPos, setBackgroundPos] = useState<BackgroundPos | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    function update() {
      ...
      for (const selection of selections) {
        const elems = [
          ...traceElemSaved.querySelectorAll(
            `[data-line="${selection.start.line}"]`,
          ),
          ...traceElemSaved.querySelectorAll(
            `[data-line="${selection.end.line}"]`,
          ),
        ] as HTMLElement[];
        for (const elem of elems) {
          ...
          bottom = Math.max(
            bottom,
            elem.offsetTop + elem.offsetHeight - traceTop,
          );
        }
      }
    ...
    <div
      ...
      className={clsy(
        ...
        detailsMode === "grid" &&
          ...
          "inline-grid grid-cols-[fit-content(0%)_2fr_3fr_0px]",
      )}
    >
      {backgroundPos && (
        <div
          data-dbg="TraceV (selection background)"
          className={`
          absolute w-[200vw]
          -mx-3
          bg-zinc-700
